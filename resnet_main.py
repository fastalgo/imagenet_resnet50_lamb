...
flags.DEFINE_float('learning_rate', 1e-3, 'The initial learning rate.')
...
def resnet_model_fn(features, labels, mode, params):
  ...
  if mode == tf.estimator.ModeKeys.TRAIN:
    ...
    # the train operation.
    tf.logging.info("****************************** the batch_size is " +str(FLAGS.train_batch_size)+ " **************************************")
    tf.logging.info("****************************** the learning_rate is " +str(FLAGS.learning_rate)+ " **************************************")
    tf.logging.info("****************************** the train_steps is " +str(FLAGS.train_steps)+ " **************************************")
    tf.logging.info("****************************** the poly_power is " +str(FLAGS.poly_power)+ " **************************************")
    ...
    with tf.control_dependencies(update_ops):
      ...
      #def host_call_fn(gs, loss, lr, ce):
        ...
        # once per loop.
        #with summary.create_file_writer(FLAGS.model_dir, max_queue=FLAGS.iterations_per_loop).as_default():
